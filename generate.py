...
def register_artifacts(artifacts: Artifacts) -> Dict[str, object]:
    ...
    for png in sorted(exported.glob("*.png")):
        images.append(artifacts.register(png, f"images/{png.name}"))

    # The two rendered feature models.
    images.append(artifacts.register(
...
def render_feature_model(assets: Artifacts) -> None:
    ...
    body = [
        ...
        f"The model captures the variability of microservice identification and "
        f"refactoring approaches: <b>{len(nodes)}</b> features "
        f"({len(groups)} groups, {len(leaves)} leaves) under the root "
        f"<i>{html.escape(tree.name)}</i>, with <b>{len(constraints)}</b> "
        f"cross-tree constraints.\n",

        figure("The complete feature model, rendered from FeatureIDE. The image "
               "is very wide — click to open it full screen, then drag to pan "
               "and scroll to zoom.",
    ...
    body.append("\n## Colour profiles\n")
    body.append(
        "One profile per tool, plus the merged profiles. Each is a FeatureIDE "
        "colour profile that paints the model with that tool's mapping; drop it "
        "into the project's <code>.profiles/</code> directory to view it in "
        "FeatureIDE. The <i>reviewed</i> column is the authors' corrected "
        "version of the automated mapping.\n")

    rows = []
    for tool in TOOLS:
        rows.append(_profile_row(assets, TOOL_LABEL[tool], tool))
    for key, _prefix, label in MERGED:
        review = ("mono2micro_user_review+Micro2Micro_user_review"
                  if key == "mono2micro+Micro2Micro" else "")
        rows.append(_profile_row(assets, label, key, review))
    body.append(table(["Mapping", "Automated", "Reviewed"], rows))
...
def _profile_row(assets: Artifacts, label: str, stem: str,
                 review_stem: str = "") -> Tuple[str, str, str]:
